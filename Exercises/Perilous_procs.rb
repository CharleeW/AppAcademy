...
# # If the number argument is not passed in, then the the elements should be run 
# # through the block once.
...
# # then the suffixes should be appended in the order that they appear in 
# # the input hash.
...
# # any number of additional procs as arguments. The method should 
# # return a hash where the keys correspond to the number of procs passed in.

# # For example, if three procs are passed in, then the hash should 
# # have the keys 1, 2, and 3.
# # The values associated with each key should be an array 
# # containing the elements of the input array that return true when 
# # passed into the corresponding proc.

# # For example, this means that the array that corresponds to the 
# # key 2 should contain the elements that return true when passed 
...
# # arguments. The hash contains procs as both keys and values. 
# # The method should return a new sentence where each word of the 
# # input sentence is changed by a value proc if the original word 
# # returns true when passed into the key proc. If an original word 
...
# # and a hash as arguments. The hash contains procs as both 
# # keys and values. The method should return a new sentence 
